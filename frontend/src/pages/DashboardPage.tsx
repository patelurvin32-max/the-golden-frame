...
import { reportService, tableService, billingService } from '@/services';
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {statsLoading ? (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[...Array(isBranchManager ? 4 : 7)].map((_, i) => <Skeleton key={i} className="h-28 rounded-2xl" />)}
        </div>
      ) : (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard title="Total Pending Payments" value={formatCurrency((billsData || []).reduce((sum: number, bill: any) => sum + (bill.total || 0), 0))} icon="⏳" color="from-amber-500/30 to-amber-600/10" />
          {!isBranchManager && (
            <>
